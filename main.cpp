...
//loss_multiclass_log  loss_binary_log
using net_type = loss_multiclass_log<fc<2,avg_pool_everything<//1000->120
                            level1<
                            level2<
                            level3<
                            level4<
                            max_pool<3,3,2,2,relu<bn_con<con<64,7,7,2,2,
                            input_rgb_image_sized<227,227>
                            >>>>>>>>>>>;

// testing network type (replaced batch normalization with fixed affine transforms)
using anet_type = loss_multiclass_log<fc<2,avg_pool_everything<  //1000->120
                            alevel1<
                            alevel2<
                            alevel3<
                            alevel4<
...
std::vector<image_info> get_imagenet_val_listing(
    const std::string& imagenet_root_dir,//validation picture root path
    const std::string& validation_images_file//validation label and pictrue name
)
{
...
//    }
    ...
    auto listing = get_imagenet_train_listing("/home/aitian/QtProject/dlib_resnet/data/trains");
    cout << "images in dataset: " << listing.size() << endl;
    const auto number_of_classes = listing.back().numeric_label+1;
    if (listing.size() == 0 || number_of_classes != 2)//1000->120
    {
       cout << "Didn't find the imagenet dataset. " << endl;
       return 1;
   }

    set_dnn_prefer_smallest_algorithms();   //Dlib底层用的是cuDNN，cuDNN其中的一个特性
                                            //是选择使用少RAM的慢方法和用多RAM的快方法。
                                            //如果你发现你你耗尽了你显卡上的内存RAM,你就
                                            //可以调用这个函数并且请求更多的节约RAM的cuDNN算法
    ...
    trainer.set_synchronization_file("imagenet_trainer_state_file.dat", std::chrono::minutes(1));//10->1
   ...
   // with a smaller value but if you aren't in a hurry this value will surely work well.
    ...
    auto f = [&data, &listing,&index](time_t seed)
    {
        ...
        while(data.is_enabled())
        {
            ...
            index++;
            //std::cout<<temp.first.filename<<std::endl;
            //image_window my_window2(img, "Original Image");
            randomly_crop_image(img, temp.second, rnd);
            //save_jpeg(temp.second,"/home/aitian/QtProject/dlib_resnet/data/crops/"+to_string(index)+".jpg");
            data.enqueue(temp);
        }
    };
    ...
    while(trainer.get_learning_rate() >= initial_learning_rate*1e-7)
    {
        samples.clear();
        labels.clear();
...
//    dlib::sleep(1000);
//    //deserialize("imagenet_trainer_state_file.dat") >> net;
...
//    for (auto l : get_imagenet_val_listing("/home/aitian/QtProject/dlib_resnet/data/vals","/home/aitian/QtProject/dlib_resnet/data/validation.txt"))
//    {
...
//        else
//            ++num_wrong_top1;
//        std::cout<<"index_of_max(p) is "<<index_of_max(p)<<"-> true is "<<l.numeric_label<<std::endl;
